...
/* ---------- MATRIX ---------- */
function matrix(){const chars=['♔','♕','♖','♗','♘','♙','e4','Nf3','ACFA','2026','64','12'];for(let i=0;i<18;i++){let d=document.createElement('div');d.className='matrix-col';d.style.left=(i*6+Math.random()*3)+'%';d.style.animationDuration=(11+Math.random()*16)+'s';d.style.animationDelay=(-Math.random()*18)+'s';d.textContent=Array.from({length:18},()=>chars[Math.floor(Math.random()*chars.length)]).join('  ');$('#matrix').appendChild(d)}}matrix();
...
function ratingPairs(stats){
  ...
  return Object.entries(names).map(([k,n])=>{
    const obj=stats?.[k],rating=obj?.best?.rating ?? obj?.last?.rating;
    return [n,rating];
...
async function enrich(m){
  try{
    ...
    return {...m,...p,_rating:highest(s),_ratings:ratingPairs(s)};
  }catch{return {...m,_rating:null,_ratings:[]}}
}
...
async function loadOwner(){
  try{
    ...
    $('#ownerRating').textContent='Highest rating: '+hi;
...
async function loadRecord(){
  const status=$('#recordStatus');status.textContent='Refreshing verified Chess.com team-match record…';
  try{
    ...
    status.textContent=t?`Verified PubAPI team record • ${t} completed events • refreshed ${new Date().toLocaleTimeString([],{hour:'numeric',minute:'2-digit'})}`:'No completed team matches were returned by Chess.com.';
  }catch(e){
