...
function matrixBoard(){const host=$('#matrixBoard');if(!host)return;const chars=['♔','♕','♖','♗','♘','♙','e4','Nf3','Bb5','O-O','ACFA','2026','64','12'];for(let i=0;i<12;i++){const d=document.createElement('div');d.className='matrix-board-col';d.style.left=(i*8.4)+'%';d.style.animationDuration=(8+Math.random()*8)+'s';d.style.animationDelay=(-Math.random()*10)+'s';d.textContent=Array.from({length:20},()=>chars[Math.floor(Math.random()*chars.length)]).join('\n');host.appendChild(d)}}matrixBoard();
